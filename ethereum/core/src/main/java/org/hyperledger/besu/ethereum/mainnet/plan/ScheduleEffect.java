 ...
 * core-defined hard-forks and externally contributed customizations live in one list: core
 * contributes full built-in hard-fork definitions, a contributor (typically a plugin) contributes a
 * modifier over the prior (floor) spec.
 */
public sealed interface ScheduleEffect
   ...
   * activation). This is the contributor case; the modifier is composed after — never replacing —
   * the floor spec's own modifications (see {@link
   * org.hyperledger.besu.ethereum.mainnet.ProtocolSpecAdapters#composedWith}).
   *
