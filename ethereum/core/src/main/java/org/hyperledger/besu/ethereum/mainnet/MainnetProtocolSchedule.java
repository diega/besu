...
public class MainnetProtocolSchedule {
   ...
   * @param balConfiguration configuration related to block access lists
   * @param metricsSystem A metricSystem instance to expose metrics in the underlying calls
   * @param serviceManager the service manager to query for plugin-provided customizers, if present
   * @return A configured mainnet protocol schedule
   */
  public static ProtocolSchedule fromConfig(
      final GenesisConfigOptions config,
  ...
  private static ProtocolSpecAdapters buildAdapters(
      final GenesisConfigOptions config, final Optional<ServiceManager> serviceManager) {
    final Map<Long, Function<ProtocolSpecBuilder, ProtocolSpecBuilder>> baseModifiers =
        new HashMap<>();
    baseModifiers.put(0L, Function.identity());
    return ProtocolScheduleCustomizer.composeAdapters(baseModifiers, config, serviceManager);
  }
