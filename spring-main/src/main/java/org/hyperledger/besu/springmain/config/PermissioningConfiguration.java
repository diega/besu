...
public class PermissioningConfiguration {
  ...
  //        final Collection<EnodeURL> fixedNodes = getFixedNodes(bootnodesAsEnodeURLs,
  // staticNodes);
  //
  //        if (permissioningConfiguration.isPresent()) {
  //            final org.hyperledger.besu.ethereum.permissioning.PermissioningConfiguration
  // configuration = this.permissioningConfiguration.get();
  ...
  public PeerPermissions peerPermissions(
      final NodePermissioningController nodePermissioningController,
      final Blockchain blockchain,
      final DiscoveryConfiguration discoveryConfiguration,
      final PeerPermissionsDenylist peerPermissionsDenylist) {
    return Optional.of(nodePermissioningController)
        .map(
            nodePC ->
                new PeerPermissionsAdapter(
                    nodePC, discoveryConfiguration.getBootnodes(), blockchain))
