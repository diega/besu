...
public class EthProtocolManagerConfiguration {
  ...
  @Bean
  public EthProtocolManager ethProtocolManager(
  ...
  EthNetworkConfig ethNetworkConfig(
      ...
      final GenesisProperties genesisProperties)
      throws IOException {
    ...
    if (genesisFile(genesisProperties.getGenesisFilename()) != null) {
      ...
      builder.setGenesisConfig(
          Resources.toString(
              genesisFile(genesisProperties.getGenesisFilename()).toURI().toURL(), UTF_8));
    ...
    //        if (p2PDiscoveryOptionGroup.bootNodes != null) {
    //            try {
    //                listBootNodes = buildEnodes(p2PDiscoveryOptionGroup.bootNodes,
  ...
  SynchronizerConfiguration synchronizerConfiguration(
      final SyncMode syncMode, final BesuProperties besuProperties) {
    return SynchronizerOptions.create()
        .toDomainObject()
        .syncMode(syncMode)
  ...
  MiningParameters miningParameters(
      final MinerOptionProperties minerOptionProperties,
      final MiningOptionsProperties miningOptionsProperties,
  ...
  protected ConsensusContextFactory consensusContextFactory(
      final GenesisConfigOptions genesisConfigOptions, final SyncState syncState) {
    {
      return (blockchain, worldStateArchive, protocolSchedule) -> {
        ...
        if (terminalBlockNumber.isPresent() && terminalBlockHash.isPresent()) {
          Optional<BlockHeader> termBlock =
              blockchain.getBlockHeader(terminalBlockNumber.getAsLong());
  ...
  SyncState syncState(
    ...
    return new SyncState(
        blockchain, ethPeers, !SyncMode.isFullSync(syncConfig.getSyncMode()), Optional.empty());
  }
  ...
  EthScheduler ethScheduler(
      final SynchronizerConfiguration syncConfig, final MetricsSystem metricsSystem) {
    return new EthScheduler(
  ...
  TransactionPoolConfiguration transactionPoolConfiguration(
      final TXPoolProperties txPoolProperties) {
