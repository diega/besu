...
public class JsonRPCConfiguration {
  ...
  public JsonRpcConfiguration jsonRpcConfiguration(
      final JsonRPCHttpProperties jsonRPCHttpProperties,
      final P2PProperties p2PProperties,
      final EngineRpcProperties engineRpcProperties) {
    ...
    //                    commandLine,
    //                    "Unable to authenticate JSON-RPC HTTP endpoint without a supplied
    // credentials file or authentication public key file");
    //        }
    ...
    jsonRpcConfiguration.setCorsAllowedDomains(
        Arrays.asList(jsonRPCHttpProperties.getCorsAllowedOrigins()));
    jsonRpcConfiguration.setRpcApis(
        Arrays.stream(jsonRPCHttpProperties.getApis()).distinct().collect(Collectors.toList()));
    jsonRpcConfiguration.setNoAuthRpcApis(
        Arrays.stream(jsonRPCHttpProperties.getNoAuthApis())
            .distinct()
            .collect(Collectors.toList()));
    jsonRpcConfiguration.setHostsAllowlist(
        Arrays.stream(engineRpcProperties.getAllowList()).distinct().collect(Collectors.toList()));
    jsonRpcConfiguration.setAuthenticationEnabled(jsonRPCHttpProperties.isAuthenticationEnabled());
    jsonRpcConfiguration.setAuthenticationCredentialsFile(
        jsonRPCHttpProperties.getAuthenticationCredentialsFile());
